...
/**
 * Nota TÃÂÃÂ©cnica: Sempre criamos uma nova instÃÂÃÂ¢ncia do cliente antes da chamada
 * para garantir o uso da API Key mais recente injetada pelo diÃÂÃÂ¡logo de seleÃÂÃÂ§ÃÂÃÂ£o.
 */
...
export const generateImage = async (config: GenerationConfig): Promise<string> => {
  ...
  try {
    ...
    throw new Error("Dados de imagem nÃÂÃÂ£o encontrados na resposta.");
  } catch (error: any) {
...
export const enhanceImage = async (base64Image: string, mimeType: string, config: EnhancementConfig): Promise<string> => {
  ...
  // Mapeamento de instruÃÂÃÂ§ÃÂÃÂµes detalhadas para o modelo
  ...
  // Prompt mais "autoritÃÂÃÂ¡rio" para forÃÂÃÂ§ar a ediÃÂÃÂ§ÃÂÃÂ£o da imagem
  ...
  try {
    ...
    throw new Error("O modelo nÃÂÃÂ£o retornou a imagem processada. Tente reduzir o nÃÂÃÂºmero de opÃÂÃÂ§ÃÂÃÂµes selecionadas.");
  } catch (error: any) {
    console.error("Gemini Image Enhancement Error:", error);
    throw new Error(error.message || "NÃÂÃÂ£o foi possÃÂÃÂ­vel melhorar a imagem agora.");
  }
};

export const animateImage = async (config: AnimationConfig, onProgress?: (msg: string) => void): Promise<string> => {
  // ObrigatÃÂÃÂ³rio: Novo cliente por chamada para capturar a chave ativa
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY || 'FAKE_API_KEY_FOR_DEVELOPMENT' });
  const base64Data = config.image.split(',')[1] || config.image;
  
  // CustomizaÃÂÃÂ§ÃÂÃÂ£o do prompt para influenciar a duraÃÂÃÂ§ÃÂÃÂ£o e fluidez
  ...
  try {
    ...
    // Polling da operaÃÂÃÂ§ÃÂÃÂ£o com mensagens dinÃÂÃÂ¢micas
    let pollCount = 0;
    while (!operation.done) {
      ...
      try {
        ...
        if (pollCount === 2) onProgress?.("Iniciando renderizaÃÂÃÂ§ÃÂÃÂ£o de quadros...");
        if (pollCount === 5) onProgress?.(`Processando animaÃÂÃÂ§ÃÂÃÂ£o de ${config.duration}s...`);
        if (pollCount > 8) onProgress?.("Otimizando compressÃÂÃÂ£o de vÃÂÃÂ­deo...");
      } catch (e: any) {
        if (e.message?.includes("404") || e.message?.includes("not found")) {
           throw new Error("ENTITY_NOT_FOUND");
        }
    ...
    if (!downloadLink) throw new Error("O servidor nÃÂÃÂ£o retornou um link de download vÃÂÃÂ¡lido.");
    ...
    if (!response.ok) throw new Error("Falha ao baixar vÃÂÃÂ­deo gerado.");
    ...
    if (error.message === "ENTITY_NOT_FOUND") {
      throw new Error("CHAVE_INVALIDA: O projeto desta chave de API nÃÂÃÂ£o possui acesso ao modelo Veo ou o faturamento estÃÂÃÂ¡ inativo.");
    }
