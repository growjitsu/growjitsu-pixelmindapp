...
const ImageAnimator: React.FC<ImageAnimatorProps> = ({ initialImage }) => {
  ...
  const fetchQuota = async () => {
    ...
    if (session?.user) {
      const status = await getQuotaStatus(session.user.id, 'video');
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start animate-fadeIn">
      ...
      <div className="lg:col-span-5 space-y-6 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
        ...
        <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
           <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${isQuotaExhausted ? 'bg-red-500' : 'bg-brand-blue animate-pulse'}`}></div>
              <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Créditos de Vídeo</span>
           </div>
           <span className={`text-xs font-black ${isQuotaExhausted ? 'text-red-500' : 'text-brand-blue'}`}>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[10px] font-bold bg-brand-blue/10 text-brand-blue px-2 py-1 rounded-md uppercase tracking-wider">Veo 3.1 Pro</span>
        </div>
        ...
        <div>
          ...
          <div 
            ...
            className={`group cursor-pointer w-full aspect-video bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl flex flex-col items-center justify-center hover:border-brand-purple transition-all overflow-hidden relative ${loading || isQuotaExhausted ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {selectedImage ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                disabled={loading || isQuotaExhausted}
                className="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-brand-purple disabled:opacity-50"
              >
        ...
        {isQuotaExhausted && (
          <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl text-amber-700 dark:text-amber-400 text-xs font-medium text-center">
             Limite de 10 vídeos diários atingido. O seu saldo resetará em 24h.
          </div>
        )}
        ...
        >
          {loading ? (
            ...
            <>
              ...
              {isQuotaExhausted ? 'Limite Diário Atingido' : 'Dar Vida à Imagem'}
            </>
          )}
        </button>
      </div>
      ...
      <div className="lg:col-span-7 flex flex-col items-center justify-center min-h-[500px] lg:min-h-[640px] bg-slate-100 dark:bg-slate-800/50 rounded-3xl border-2 border-dashed border-slate-300 dark:border-slate-700 transition-colors relative overflow-hidden">
        {/* ... Rest of the result display logic (unchanged) ... */}
        {!resultVideo && !loading && (
          <div className="text-center p-12">
            ...
            <p className="text-sm text-slate-500 max-w-xs mx-auto mt-3">O limite diário ajuda a manter a plataforma gratuita para todos.</p>
          </div>
        )}
        ...
        {resultVideo && (
          <div className={`w-full h-full flex flex-col items-center justify-center p-6 ${aspectRatio === '9:16' ? 'max-w-[360px] mx-auto' : 'w-full max-w-5xl'}`}>
             <div className="relative w-full rounded-[2rem] overflow-hidden shadow-2xl bg-black group border border-slate-200 dark:border-slate-800">
                <video src={resultVideo} autoPlay loop playsInline controls className="w-full h-auto max-h-[70vh] block" />
             </div>
          </div>
        )}
      </div>
